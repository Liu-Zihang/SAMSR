...
from z_rubbish.meta_1 import generate_masks_from_batch,load_sam_model,generate_weighted_noise,count_masks_per_pixel, merge_all_mask_to_one_RoPE_batch,normalize_noise_to_unit_variance

def normalize_mask_counts(mask_count: th.Tensor) -> th.Tensor:
    """
    归一化每个像素点的掩码数量到 [0, 1]，基于每张图像的最大掩码数量。

    参数：
        mask_count: 形状为 (B, H, W) 的张量，每个值表示该像素被覆盖的次数。

    返回：
        normalized_weights: 形状为 (B, H, W) 的归一化权重张量。
    """
    # 计算每张图像的最大掩码数量
    max_masks_per_image = mask_count.view(mask_count.size(0), -1).max(dim=1)[0].view(-1, 1, 1)  # (B, 1, 1)

    # 防止除以零
    max_masks_per_image = th.clamp(max_masks_per_image, min=1.0)

    # 归一化掩码数量
    normalized_weights = mask_count / max_masks_per_image  # (B, H, W)
...
def _extract_into_tensor_1(tensor: th.Tensor, timesteps: th.Tensor, broadcast_shape):
    """
    从一个张量中提取一批索引对应的值，并扩展到目标形状。

    :param tensor: 一维的 PyTorch 张量。
    :param timesteps: 一个包含索引的张量，用于从数组中提取值。
    :param broadcast_shape: 需要广播到的目标形状，包含 K 个维度，第一维等于批量大小。
    :return: 形状为 [batch_size, 1, ...] 的张量，维度数为 K。
    """
...
def array_to_tensor(value, name, num_timesteps, device):
    """
    将 value 转换为长度为 num_timesteps 的 PyTorch 张量。
    - 如果 value 本身是 Tensor，直接返回。
    - 如果是 float 或 int，返回一个全值张量。
    - 如果是 np.ndarray，转换为 float Tensor。
    否则抛出 TypeError。
    """
    if isinstance(value, th.Tensor):
        return value.to(device=device)
    if isinstance(value, (float, int)):
        return th.full((num_timesteps,), float(value), device=device)
    if isinstance(value, np.ndarray):
        return th.from_numpy(value).to(device=device).float()
    raise TypeError(f"{name} 应该是一个 PyTorch 张量，但得到的是 {type(value)}")
...
class GaussianDiffusion:
    ...
    def reset_weight(self, masks_tensor, t, y, device):
        """
        根据masks动态调整扩散参数，保持参数的1维结构
        """
        # 确保原始参数为tensor格式（但保持1维）
        if not isinstance(self.kappa, th.Tensor):
            if isinstance(self.kappa, (float, int)):
                ...
                raise TypeError(f"self.kappa 应该是一个 PyTorch 张量，但得到的是 {type(self.kappa)}")
        ...
        if not isinstance(self.sqrt_etas, th.Tensor):
            if isinstance(self.sqrt_etas, (float, int)):
                ...
                raise TypeError(f"self.sqrt_etas 应该是一个 PyTorch 张量，但得到的是 {type(self.sqrt_etas)}")
        ...
        # 计算etas_prev_modified - 需要处理边界情况
        batch_size, channels, height, width = self.etas_modified.shape
        
        # 为t-1时间步提取参数
        t_prev = th.clamp(t - 1, min=0)  # 防止负数索引
        ...
        # 计算posterior相关参数
        self.posterior_variance_modified = (self.kappa_modified**2 * self.etas_prev_modified / 
                                        self.etas_modified * self.alpha_modified)
        
        # 处理posterior_variance_clipped - 避免在t=0时为0
        self.posterior_variance_clipped_modified = th.where(
            self.posterior_variance_modified > 0,
            self.posterior_variance_modified,
            th.ones_like(self.posterior_variance_modified) * 1e-8  # 小的正数避免log(0)
        )
        ...
        if self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.RESIDUAL]:
            self.weight_loss_mse_modified = 0.5 / self.posterior_variance_clipped_modified * (self.alpha_modified / self.etas_modified)**2
        elif self.model_mean_type in [ModelMeanType.EPSILON, ModelMeanType.EPSILON_SCALE]  :
            self.weight_loss_mse_modified = 0.5 / self.posterior_variance_clipped_modified * (
                    self.kappa_modified * self.alpha_modified / ((1-self.etas_modified) * self.sqrt_etas_modified)
                    )**2
    ...
    def q_mean_variance(self, x_start, y, t):
        ...
        """
        mean = _extract_from_tensor(self.etas, t, x_start.shape) * (y - x_start) + x_start
        variance = _extract_from_tensor(self.etas, t, x_start.shape) * self.kappa**2
    ...
    def q_sample(self, x_start, y, t, noise=None):
        ...
        return (
            _extract_from_tensor(self.etas, t, x_start.shape) * (y - x_start) + x_start
            + _extract_from_tensor(self.sqrt_etas * self.kappa, t, x_start.shape) * noise
        )
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        posterior_mean = (
            _extract_from_array(self.posterior_mean_coef1, t, x_t.shape) * x_t
            + _extract_from_array(self.posterior_mean_coef2, t, x_t.shape) * x_start
        )
        posterior_variance = _extract_from_array(self.posterior_variance, t, x_t.shape)
        posterior_log_variance_clipped = _extract_from_array(
    ...
    def p_mean_variance(
        ...
        B, C = x_t.shape[:2]
        assert t.shape == (B,)
        model_output = model(self._scale_input_before(x_t, t), t, **model_kwargs)

        model_variance = _extract_from_array(self.posterior_variance, t, x_t.shape)
        model_log_variance = _extract_from_array(self.posterior_log_variance_clipped, t, x_t.shape)

        # ddim coef
        ddim_coef1 = _extract_from_array(self.ddim_coef1, t, x_t.shape) # etas_pre*etas
        ddim_coef2 = _extract_from_array(self.ddim_coef2, t, x_t.shape) # etas_pre/etas
        etas = _extract_from_array(self.etas, t, x_t.shape)
        etas_prev = _extract_from_array(self.etas_prev, t, x_t.shape)
    ...
    def _predict_xstart_from_eps(self, x_t, y, t, eps):
        ...
        return  (
            x_t - _extract_from_array(self.sqrt_etas, t, x_t.shape) * self.kappa * eps
                - _extract_from_array(self.etas, t, x_t.shape) * y
        ) / _extract_from_array(1 - self.etas, t, x_t.shape)

    def _predict_xstart_from_eps_scale(self, x_t, y, t, eps):
        assert x_t.shape == eps.shape
        return  (
            x_t - eps - _extract_from_array(self.etas, t, x_t.shape) * y
        ) / _extract_from_array(1 - self.etas, t, x_t.shape)
    ...
    def _predict_eps_from_xstart(self, x_t, y, t, pred_xstart):
        return (
            x_t - _extract_from_array(1 - self.etas, t, x_t.shape) * pred_xstart
                - _extract_from_array(self.etas, t, x_t.shape) * y
        ) / _extract_from_array(self.kappa * self.sqrt_etas, t, x_t.shape)
    ...
    def prior_sample(self, y, masks_tensor, noise=None):
        """
        ...
        return y + _extract_from_array(self.kappa * self.sqrt_etas, t, y.shape) * noise
        ...
        if True:
            ...
            if self.loss_type == LossType.MSE or self.loss_type == LossType.WEIGHTED_MSE:
                ...
                target = {
                    ...
                    ModelMeanType.EPSILON_SCALE: noise*self.kappa*_extract_from_array(self.sqrt_etas, t, noise.shape),
                ...
                if self.model_mean_type == ModelMeanType.EPSILON_SCALE:
                    terms[loss_type] /= (self.kappa**2 * _extract_from_array(self.etas, t, t.shape))
                if self.loss_type == LossType.WEIGHTED_MSE:
                    weights = _extract_from_array(self.weight_loss_mse, t, t.shape)
                else:
                    weights = 1
    ...
    def _scale_input_train(self, inputs, t):
        if self.normalize_input:
            if self.latent_flag:
                # the variance of latent code is around 1.0
                std = th.sqrt(_extract_from_array(self.etas, t, inputs.shape) * self.kappa**2 + 1)
                inputs_norm = inputs / std
            else:
                inputs_max = _extract_from_array(self.sqrt_etas, t, inputs.shape) * self.kappa * 3 + 1
    ...
    def _scale_input_before(self, inputs, t):
        if self.normalize_input:
            if self.latent_flag:
                # the variance of latent code is around 1.0
                std = th.sqrt(_extract_from_array(self.etas, t, inputs.shape) * self.kappa**2 + 1)
    ...
    def _scale_input(self, inputs, t):
        """
        兼容版本的 _scale_input，能处理 tensor 和 numpy 参数
        """
        if self.normalize_input:
            if self.latent_flag:
                # 使用修改后的参数（如果可用）
                if hasattr(self, 'etas_modified') and hasattr(self, 'kappa_modified'):
                    ...
                    std = th.sqrt(self.etas_modified * self.kappa_modified**2 + 1)
                    inputs_norm = inputs / std
                else:
                    print("Using none-modified weight!")
                    # 使用修复的提取函数
                    etas_val = _extract_from_tensor(self.etas, t, inputs.shape)
                    if isinstance(self.kappa, th.Tensor):
                        kappa_val = _extract_from_tensor(self.kappa, t, inputs.shape)
                    else:
                        kappa_val = self.kappa
                    std = th.sqrt(etas_val * kappa_val**2 + 1)
                    inputs_norm = inputs / std
            else:
                if hasattr(self, 'sqrt_etas_modified') and hasattr(self, 'kappa_modified'):
                    inputs_max = self.sqrt_etas_modified * self.kappa_modified * 3 + 1
                    inputs_norm = inputs / inputs_max
                else:
                    # 使用修复的提取函数
                    sqrt_etas_val = _extract_from_tensor(self.sqrt_etas, t, inputs.shape)
                    if isinstance(self.kappa, th.Tensor):
                        kappa_val = _extract_from_tensor(self.kappa, t, inputs.shape)
                    else:
                        kappa_val = self.kappa
                    inputs_max = sqrt_etas_val * kappa_val * 3 + 1
                    inputs_norm = inputs / inputs_max
        else:
            inputs_norm = inputs
        return inputs_norm
    ...
    def ddim_sample(
        ...
        # etas = _extract_from_tensor(self.etas, t, x.shape)
        # etas_prev = _extract_from_tensor(self.etas_prev, t, x.shape)
        # alpha = _extract_from_tensor(self.alpha, t, x.shape)
    ...
    def ddim_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                if one_step:
                    print("One step used in distillation method")
                    out["sample"]=out["pred_xstart"]
                    yield out
                    break
                yield out
    ...
    def prior_sample_modified(self, y, noise=None):
        ...
        if (hasattr(self, 'kappa_modified') and 
            hasattr(self, 'sqrt_etas_modified')):
            
            # 使用修改后的参数（取最后时间步的值）
            # 如果参数是4维的，我们需要从最后时间步提取
            device = y.device
            t_last = th.tensor([self.num_timesteps-1] * y.shape[0], device=device).long()
            
            # 如果已经有修改后的参数（4维），直接使用
            # 否则提取最后时间步的参数
            if hasattr(self, 'kappa_modified') and self.kappa_modified.dim() == 4:
                kappa_last = self.kappa_modified
                sqrt_etas_last = self.sqrt_etas_modified
            else:
                kappa_last = _extract_from_tensor(self.kappa, t_last, y.shape)
                sqrt_etas_last = _extract_from_tensor(self.sqrt_etas, t_last, y.shape)
    ...
    def prior_sample_train(self, y, masks_tensor=None, noise=None):
        """
        从先验分布中生成样本，即 q(x_T|x_0) ~= N(x_T|y, ~)

        :param y: 形状为 [N x C x H x W] 的降解输入张量。
        :param masks_tensor: 形状为 [N x M x H x W] 的掩码张量。如果为 None，则自动生成。
        :param noise: 形状为 [N x C x H x W] 的噪声张量。如果为 None，则自动生成。
        :return: 形状为 [N x C x H x W] 的采样张量。
        """
        kappa = self.kappa
        if not isinstance(kappa, th.Tensor):
            if isinstance(kappa, (float, int)):
                ...
                #print(f"Converted self.kappa from {type(kappa)} to torch.Tensor with shape {kappa.shape}")
            elif isinstance(kappa, np.ndarray):
                kappa = th.from_numpy(kappa).to(device=y.device).float()
                #print(f"Converted self.kappa from numpy.ndarray to torch.Tensor with shape {self.kappa.shape}")
            else:
                raise TypeError(f"self.kappa 应该是一个 PyTorch 张量，但得到的是 {type(kappa)}")
        ...
        if not isinstance(sqrt_etas, th.Tensor):
            if isinstance(sqrt_etas, (float, int)):
                ...
                #print(f"Converted self.sqrt_etas from {type(sqrt_etas)} to torch.Tensor with shape {sqrt_etas.shape}")
            elif isinstance(sqrt_etas, np.ndarray):
                sqrt_etas = th.from_numpy(sqrt_etas).to(device=y.device).float()
                #print(f"Converted self.sqrt_etas from numpy.ndarray to torch.Tensor with shape {sqrt_etas.shape}")
            else:
                raise TypeError(f"self.sqrt_etas 应该是一个 PyTorch 张量，但得到的是 {type(sqrt_etas)}")
        ...
        # 提取 self.kappa 和 self.sqrt_etas
        kappa_extracted = _extract_into_tensor_1(kappa, t, y.shape)  # (B, C, H, W)
        sqrt_etas_extracted = _extract_into_tensor_1(sqrt_etas, t, y.shape)  # (B, C, H, W)

        # 归一化权重 W 的形状为 (B, H, W)，需要扩展到 (B, 1, H, W)
        W_expanded = W.unsqueeze(1)  # (B, 1, H, W)
        W_expanded = W_expanded / 5

        # 调整 kappa 和 sqrt_etas

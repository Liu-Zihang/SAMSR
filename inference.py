...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
            ...
            type=str,
            default="SamSR",
            choices=["SamSR",'realsrx4', 'bicsrx4_opencv', 'bicsrx4_matlab', 'SinSR'],
            help="Chopping forward.",
            )
...
def get_configs(args):
    if args.config is None:
        if args.task == "SamSR":
            configs = OmegaConf.load('configs/SamSR.yaml')
    else:
    ...
    if args.ckpt is None:
        if not ckpt_dir.exists():
            ckpt_dir.mkdir()
        if args.task == "SamSR":
            ckpt_path = ckpt_dir / f'SamSR_v1.pth'
    else:
    ...
    if not ckpt_path.exists():
        if args.task == "SamSR":
            load_file_from_url(
                url=f"https://huggingface.co/zh-liu799/SAMSR/resolve/main/SamSR_v1.pth",
                model_dir=ckpt_dir,
                progress=True,

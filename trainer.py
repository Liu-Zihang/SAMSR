...
class TrainerBase:
    ...
    def training_step(self, sam_model, data):
        pass
...
class TrainerDifIR(TrainerBase):
    ...
    def training_step(self, sam_model, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            print(f"noise shape in trainer is:{noise.shape} ")
    ...
    def validation(self, sam_model, phase='val'):
        if self.rank == 0:
...
class TrainerDistillDifIR(TrainerDifIR):
    ...
    def validation(self, sam_model, phase='val'):
        # Only evaluated the result of the first step
        if self.rank == 0:
            if self.configs.train.use_ema_val:
                self.reload_ema_model()
                self.ema_model.eval()
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                # Normalize the results before passing to metrics
                results = results.detach() * 0.5 + 0.5  # Scale to [0, 1]
                im_lq = im_lq * 0.5 + 0.5  # Normalize low-quality images to [0, 1]
                if 'gt' in data:
                    im_gt = im_gt * 0.5 + 0.5  # Normalize ground truth images to [0, 1]

                # Clamp the values to ensure they are within [0, 1]
                results = results.clamp(0, 1)
                im_lq = im_lq.clamp(0, 1)
                if 'gt' in data:
                    im_gt = im_gt.clamp(0, 1)

                if 'gt' in data:
                    mean_psnr += util_image.batch_PSNR(results, im_gt, ycbcr=True)
                    mean_lpips += self.lpips_loss(results, im_gt).sum().item()

                with torch.no_grad():
                    mean_clipiqa += self.metric_dict["clipiqa"](results).sum().item()
                    mean_musiq += self.metric_dict["musiq"](results).sum().item()

                if (ii + 1) % self.configs.train.log_freq[2] == 0:
                    self.logger.info(f'Validation: {ii+1:02d}/{num_iters_epoch:02d}...')
                    x2 = vutils.make_grid(results, normalize=True, scale_each=True)
                    if self.configs.train.save_images:
                        util_image.imwrite(
                            x2.cpu().permute(1, 2, 0).numpy(),
                            self.image_dir / phase / f"predict_x_{self.log_step_img[phase]:05d}.png",
